				...
				<tbody>
					...
					?>
						<tr>
							...
							<td class="text-center">
                                <?php if($row['status'] == 0): ?>
                                    <span class="badge badge-light">Tertunda</span>
                                <?php elseif($row['status'] == 1): ?>
                                    <span class="badge badge-primary">Penuh sesak</span>
								<?php elseif($row['status'] == 2): ?>
                                    <span class="badge badge-warning">Keluar untuk Pengiriman</span>
								<?php elseif($row['status'] == 3): ?>
                                    <span class="badge badge-success">Terkirim</span>
								<?php elseif($row['status'] == 5): ?>
                                    <span class="badge badge-success">Dijemput</span>
                                <?php else: ?>
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  		Action
				                    <span class="sr-only">Alihkan Tarik-turun</span>
				                  </button>
				                  <div class="dropdown-menu" role="menu">
				                    <a class="dropdown-item" href="?page=orders/view_order&id=<?php echo $row['id'] ?>">Tampilan Pemesanan</a>
									<?php if($row['paid'] == 0 && $row['status'] != 4): ?>
				                    <a class="dropdown-item pay_order" href="javascript:void(0)"  data-id="<?php echo $row['id'] ?>">Tandai sebagai Dibayar</a>
									<?php endif; ?>
